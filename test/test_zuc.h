...
TEST(test_zuc, zuc_test2)
{
    ...
    uint32_t good_ouput[2] = {0x0657cfa0, 0x7096398b};
...
TEST(test_zuc, zuc_test3)
{
    uint8_t  key[16]       = {0x3d, 0x4c, 0x4b, 0xe9, 0x6a, 0x82, 0xfd, 0xae, 0xb5, 0x8f, 0x64, 0x1d, 0xb1, 0x7b, 0x45, 0x5b};
    uint8_t  iv[16]        = {0x84, 0x31, 0x9a, 0xa8, 0xde, 0x69, 0x15, 0xca, 0x1f, 0x6b, 0xda, 0x6b, 0xfb, 0xd8, 0xc7, 0x66};
    uint32_t good_ouput[2] = {0x14f1c272, 0x3279c419};
...
TEST(test_zuc, zuc_test4)
{
    uint8_t  key[16]          = {0x4d, 0x32, 0x0b, 0xfa, 0xd4, 0xc2, 0x85, 0xbf, 0xd6, 0xb8, 0xbd, 0x00, 0xf3, 0x9d, 0x8b, 0x41};
    uint8_t  iv[16]           = {0x52, 0x95, 0x9d, 0xab, 0xa0, 0xbf, 0x17, 0x6e, 0xce, 0x2d, 0xc3, 0x15, 0x04, 0x9e, 0xb5, 0x74};
    uint32_t good_ouput[2000] = {0};
    good_ouput[0]             = 0xed4400e7;
    good_ouput[1]             = 0x0633e5c5;
    good_ouput[1999]          = 0x7a574cdb;

    uint32_t temp[2000] = {0};
    int      len        = 2000;
    ZUC(key, iv, temp, len);
    EXPECT_EQ(good_ouput[0], temp[0]);
    EXPECT_EQ(good_ouput[1], temp[1]);
    EXPECT_EQ(good_ouput[1999], temp[1999]);
}
...
TEST(test_zuc, eea3_test2)
{
    ...
    // clang-format off
    uint32_t plaintext[]  = {
        0x14a8ef69, 0x3d678507, 0xbbe7270a, 0x7f67ff50, 0x06c3525b, 0x9807e467, 0xc4e56000, 0xba338f5,
        0x42955903, 0x67518222, 0x46c80d3b, 0x38f07f4b, 0xe2d8ff58, 0x05f51322, 0x29bde93b, 0xbbdcaf3,
        0x2bf1ee97, 0x2fbf9977, 0xbada8945, 0x847a2a6c, 0x9ad34a66, 0x7554e04d, 0x1f7fa2c3, 0x3241bd8,
        0x01ba220d
    };
    uint32_t ciphertext[] = {
        0x131d43e0, 0xdea1be5c, 0x5a1bfd97, 0x1d852cbf, 0x712d7b4f, 0x57961fea, 0x3208afa8, 0x0d34845c,
        0x56ad09c7, 0x417e58bc, 0x69cf8866, 0xd1353f74, 0x865e8078, 0x1d202dfb, 0x3ecff7fc, 0x0c5a7cc4,
        0xe82a204e, 0xd0e350fc, 0x0f6f2613, 0xb2f2bca6, 0xdf5a473a, 0x57a4a00d, 0x985ebad8, 0xb1b3546f,
        0x64a07b01
    };
    // clang-format on
...
TEST(test_zuc, eea3_test3)
{
    uint8_t  key[16]   = {0xd4, 0x55, 0x2a, 0x8f, 0xd6, 0xe6, 0x1c, 0xc8, 0x1a, 0x20, 0x09, 0x14, 0x1a, 0x29, 0xc1, 0x0b};
...
TEST(test_zuc, eea3_test4)
{
    uint8_t  key[16]   = {0xdb, 0x84, 0xb4, 0xfb, 0xcc, 0xda, 0x56, 0x3b, 0x66, 0x22, 0x7b, 0xfe, 0x45, 0x6f, 0x0f, 0x77};
...
TEST(test_zuc, eia3_test1)
{
    ...
    uint32_t temp[1] = {0};
    zuc_EIA3(key, count, direction, bearer, length, message, temp);
    EXPECT_EQ(mac, temp[0]);
}

TEST(test_zuc, eia3_test2)
{
    uint8_t  key[16]   = {0x47, 0x05, 0x41, 0x25, 0x56, 0x1e, 0xb2, 0xdd, 0xa9, 0x40, 0x59, 0xda, 0x05, 0x09, 0x78, 0x50};
    uint32_t count     = 0x561eb2dd;
    uint32_t bearer    = 0x14;
    uint32_t direction = 0x0;
    uint32_t length    = 90;
    uint32_t message[] = {0x00000000, 0x00000000, 0x00000000};
    uint32_t mac       = 0x6719a088;
...
TEST(test_zuc, eia3_test3)
{
    uint8_t  key[16]   = {0xc9, 0xe6, 0xce, 0xc4, 0x60, 0x7c, 0x72, 0xdb, 0x00, 0x0a, 0xef, 0xa8, 0x83, 0x85, 0xab, 0x0a};
...
TEST(test_zuc, eia3_test4)
{
    uint8_t  key[16]   = {0xc8, 0xa4, 0x82, 0x62, 0xd0, 0xc2, 0xe2, 0xba, 0xc4, 0xb9, 0x6e, 0xf7, 0x7e, 0x80, 0xca, 0x59};
...
TEST(test_zuc, eia3_test5)
{
    uint8_t  key[16]   = {0x6b, 0x8b, 0x08, 0xee, 0x79, 0xe0, 0xb5, 0x98, 0x2d, 0x6d, 0x12, 0x8e, 0xa9, 0xf2, 0x20, 0xcb};
    uint32_t count     = 0x561eb2dd;
